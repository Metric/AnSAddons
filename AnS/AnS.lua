            ...
            if (not AuctionFrame_DoesCategoryHaveFlag("WOW_TOKEN_FLAG", AuctionFrameBrowse.selectedCategoryIndex)) then
                ...
                for i=1, NUM_BROWSE_TO_DISPLAY do
                    ...
                    if ( not shouldHide ) then
                        ...
                        -- apparently on TBC/Wotlk hasAllInfo still returns true
...
local function RegisterTSMDataSources() 
    if (not TSM_API) then
        return;
    end
...
local function RegisterTSMCustomDataSources()
    ...
    local sources = TradeSkillMasterDB['g@ @userData@customPriceSources'];
    ...
    for k,v in pairs(sources) do
        if (type(k) == "string") then
            Sources:Register(k, TSMDB.GetPrice, k);
        end
    end
end
