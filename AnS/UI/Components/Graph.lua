...
function Graph:SetFormatter(fn)
    self.formatFn = fn;
end

function Graph:OnDotEnter(f)
    local id = f:GetID();
...
function Graph:OnDotLeave()
    -- hide tooltip
    local tooltip = GameTooltip;
    tooltip:Hide();
end

function Graph:Load(x,y,point,anchor,width, height)
    local f = self.frame;
    local i;
...
function Graph:Update(data, xlabels, ylabels, max)
    self.data = data;
    self.max = max;
...
function Graph:SetRectFill(frame, fill, x, y, x2, y2)
...
function Graph:SetTriangleFill(frame, fill, asc, x, y, x2, y2)  
